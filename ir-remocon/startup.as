	...
	; Compiler options:
	;
	; -oir-remocon.cof -mir-remocon.map --summary=default --output=default \
	; ir-remocon.p1 scif_tx.p1 ir.p1 --chip=12F683 -P --runtime=default \
...
;	17 bytes of RAM objects in bank 0 to zero

	psect	init
	global	__Lrbss_0
	movlw	low(__Lrbss_0)		;load the low address into FSR
	movwf	FSR
	movlw	low(__Lrbss_0+17)		;load the high address+1 into W
...
;	14 bytes of RAM objects in common bank to zero
	...
	movwf	FSR
	movlw	low(__Lcommon+14)		;load the high address+1 into W
	fcall	clear_ram		;go and zero that range of addresses
