	...
	; Compiler options:
	;
	; -otmp-hum.cof -mtmp-hum.map --summary=default --output=default \
	; tmp-hum.p1 scif.p1 --chip=12F683 -P --runtime=default --opt=default \
...
;	16 bytes of RAM objects in bank 0 to zero

	psect	init
	global	__Lrbss_0
	movlw	low(__Lrbss_0)		;load the low address into FSR
	movwf	FSR
	movlw	low(__Lrbss_0+16)		;load the high address+1 into W
...
;	8 bytes of RAM objects in common bank to zero
	...
	movwf	FSR
	movlw	low(__Lcommon+8)		;load the high address+1 into W
	fcall	clear_ram		;go and zero that range of addresses
